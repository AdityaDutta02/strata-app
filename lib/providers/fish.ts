...
/** Same as cloneVoice but takes already-downloaded bytes — used by the onboarding pipeline
 *  once training audio has been copied into our own backup storage (R2), so this never
 *  re-touches the platform's storage host. */
export async function cloneVoiceFromBuffer(
  ...
  await logProviderCall(viewerId, 'fish.cloneVoiceFromBuffer', { name, bytes: buffer.byteLength }, token)
  if (isMockMode(token)) {
    return `mock-fish-voice-${Buffer.from(name).toString('hex').slice(0, 12)}`
  }
  const form = new FormData()
